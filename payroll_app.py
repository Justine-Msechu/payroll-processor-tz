...
# ── Developer / admin account — hidden from all end-user screens ──
# Change these to your own secret credentials before distributing.
DEVELOPER_USER = "devadmin"
DEVELOPER_PASS = "Tz@Payroll!Dev99"

# ── GitHub update settings ────────────────────────────────────
# Set GITHUB_USER and GITHUB_REPO to your GitHub details.
# Leave as "YOUR_GITHUB_USERNAME" to disable the update system.
GITHUB_USER   = "Justine-Msechu"
GITHUB_REPO   = "payroll-processor-tz"
GITHUB_BRANCH = "main"
_gh_base      = "https://raw.githubusercontent.com/" + GITHUB_USER + "/" + GITHUB_REPO + "/" + GITHUB_BRANCH
VERSION_URL   = _gh_base + "/version.txt"
APP_URL       = _gh_base + "/payroll_app.py"
EXE_URL       = "https://github.com/" + GITHUB_USER + "/" + GITHUB_REPO + "/releases/latest/download/PayrollProcessor.exe"
GITHUB_READY  = GITHUB_USER != "YOUR_GITHUB_USERNAME"
IS_FROZEN     = getattr(sys, "frozen", False)   # True when running as compiled .exe
...
def verify_login(username, password):
    ...
    if u == DEVELOPER_USER and hash_pw(password) == hash_pw(DEVELOPER_PASS):
        return "admin"
    # Regular user accounts
    rec = load_users().get(u)
    if rec and rec["hash"] == hash_pw(password):
        return rec["role"]
    return None
...
def make_print_html(records, month_label, created_by):
    ...
    for i, r in enumerate(records):
        ...
        rows_html += (
            ...
            "<td style='color:#145a32;font-weight:bold'>"
            + "{:,.2f}".format(r["amount_paid"]) + "</td>"
            "</tr>"
        )
    ...
    return (
        ...
        "</style></head><body>"
        "<div class='top'>"
        "<div><h1>PAYROLL REGISTER"
        + ("  \u2014  " + month_label if month_label else "") + "</h1></div>"
        "<div class='meta'>"
        ...
        "<div class='summary'>"
        "<div class='sc'><div class='lb'>Employees</div>"
        "<div class='vl'>" + str(len(records)) + "</div></div>"
        "<div class='sc'><div class='lb'>Total Salary</div>"
        "<div class='vl'>TZS " + "{:,.2f}".format(ts) + "</div></div>"
        "<div class='sc'><div class='lb'>Total NSSF</div>"
        "<div class='vl'>TZS " + "{:,.2f}".format(tn) + "</div></div>"
        "<div class='sc'><div class='lb'>Total PAYE</div>"
        "<div class='vl'>TZS " + "{:,.2f}".format(tp) + "</div></div>"
        "<div class='sc'><div class='lb'>Total Net Pay</div>"
        "<div class='vl'>TZS " + "{:,.2f}".format(tt) + "</div></div>"
...
def fetch_update_in_bg(holder):
    """
    Background thread — puts result into holder[0].

    Running as .exe:
        Fetches version.txt from GitHub.
        holder[0] = "same" | "error" | version_string   e.g. "1.0.2"

    Running as .py (developer):
        Fetches payroll_app.py from GitHub and compares hash.
        holder[0] = "same" | "error" | new_source_code_string
    """
    def _run():
        try:
            ...
            if IS_FROZEN:
                ...
                if _ver(remote_ver) > _ver(VERSION):
                    holder[0] = remote_ver
                else:
                    holder[0] = "same"
            else:
...
def apply_update(update_data, parent):
    """
    Apply the update.

    .exe mode  — update_data is a version string.
        Downloads new exe, writes an updater.bat that swaps the files
        after this process exits, then quits.

    .py mode   — update_data is the new source code.
        Writes it over the current .py and restarts with os.execv.
    """
    import shutil
    import urllib.request as _req

    if IS_FROZEN:
        # Download new exe alongside the current one
        this_exe    = sys.executable
        new_exe     = this_exe + ".new"
        updater_bat = os.path.join(os.path.dirname(this_exe), "updater.bat")
        ...
        dlg.geometry("380x140+" + str((sw - 380) // 2) + "+" + str((sh - 140) // 2))
        tk.Label(dlg, text="Downloading update, please wait...",
                 font=("Segoe UI", 11, "bold"), bg="#1e2a3a", fg="#2ecc71").pack(pady=(24, 6))
        ...
        try:
            _req.urlretrieve(EXE_URL, new_exe)
            dlg.destroy()

            # Build the swap script — runs after this process exits
            lines = [
                "@echo off\r\n",
                "timeout /t 2 /nobreak >nul\r\n",
                "move /Y \"" + new_exe + "\" \"" + this_exe + "\"\r\n",
                "start \"\" \"" + this_exe + "\"\r\n",
                "del \"%~f0\"\r\n",
            ]
            with open(updater_bat, "w") as bf:
                bf.writelines(lines)

            messagebox.showinfo("Update Ready",
                "The update has been downloaded.\n\n"
                "The app will now close and restart with the new version.",
                parent=parent)
            subprocess.Popen(
                ["cmd", "/c", updater_bat],
                creationflags=subprocess.CREATE_NO_WINDOW)
            parent.after(600, parent.destroy)
        ...
        # .py mode — overwrite source and restart
        try:
            this_file = os.path.abspath(__file__)
...
# ══════════════════════════════════════════════════════════════
#  FIRST LAUNCH GATE
#  Shown when no user accounts exist yet.
#  Asks: Do you have an account already?
# ══════════════════════════════════════════════════════════════
class FirstLaunchGate(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title(APP_TITLE)
        self.resizable(False, False)
        self.configure(bg="#1e2a3a")
        self.choice = None   # "create" | "login"
        self._build()
    ...
    def _build(self):
        T = THEMES["Dark Blue"]
        top = tk.Frame(self, bg="#1a5276", pady=24)
        top.pack(fill="x")
        tk.Label(top, text="💼", font=("Segoe UI", 44),
                 bg="#1a5276", fg="white").pack()
        tk.Label(top, text="PAYROLL PROCESSOR",
                 font=("Segoe UI", 17, "bold"), bg="#1a5276", fg="white").pack()
        tk.Label(top, text="Tanzania",
                 font=("Segoe UI", 9), bg="#1a5276", fg="#d5f5e3").pack(pady=(2, 0))

        card = tk.Frame(self, bg=T["PANEL"], padx=50, pady=34)
        card.pack(padx=50, pady=30)

        tk.Label(card, text="Welcome!",
                 font=("Segoe UI", 15, "bold"), bg=T["PANEL"], fg=T["ACCENT"]
                 ).pack(pady=(0, 4))
        tk.Label(card, text="Do you already have an account on this app?",
                 font=("Segoe UI", 11), bg=T["PANEL"], fg=T["TEXT"]
                 ).pack(pady=(0, 22))

        tk.Button(card,
                  text="🆕   No — Create my account now",
                  font=("Segoe UI", 11, "bold"),
                  bg=T["ACCENT2"], fg="white",
        ...
        tk.Button(card,
                  text="🔑   Yes — I have an account, take me to Login",
                  font=("Segoe UI", 11),
...
class FirstSetupWindow(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title(APP_TITLE + "  —  Create Account")
        self.resizable(False, False)
        self.configure(bg="#1e2a3a")
    ...
    def _build(self):
        ...
        tk.Label(top, text="You only need to do this once",
                 font=("Segoe UI", 9), bg="#1a5276", fg="#d5f5e3").pack(pady=(2, 0))
        ...
        for label, var, show, hint in [
            ...
            ("🔑  Confirm Password", self._conf_var, "●", "Type your password again"),
        ]:
            tk.Label(card, text=label, font=("Segoe UI", 10, "bold"),
                     bg=T["PANEL"], fg=T["TEXT"], anchor="w").pack(anchor="w", pady=(10, 1))
            tk.Entry(card, textvariable=var, show=show, font=("Segoe UI", 11),
                     bg=T["ENTRY_BG"], fg=T["ENTRY_FG"],
                     insertbackground="white", relief="flat", bd=5, width=32
                     ).pack(fill="x", ipady=6)
    ...
    def _create(self):
        ...
        if not name:
            ...
            return
        if len(uname) < 3:
            self.err.set("⚠  Username must be at least 3 characters (no spaces).")
            return
        ...
        users = load_users()
        if uname in users:
            self.err.set("⚠  That username already exists. Choose another.")
            return
...
class LoginWindow(tk.Tk):
    ...
    def _build(self):
        ...
        tk.Label(top, text="PAYROLL PROCESSOR",
                 font=("Segoe UI", 17, "bold"), bg="#1a5276", fg="white").pack()
        tk.Label(top, text="Tanzania  —  Please log in",
                 font=("Segoe UI", 9), bg="#1a5276", fg="#d5f5e3").pack(pady=(2, 0))
        ...
        for row, (icon, lbl, var, show) in enumerate([
            ...
            tk.Label(card, text=icon + "  " + lbl,
                     font=("Segoe UI", 10), bg=T["PANEL"], fg=T["TEXT"], anchor="w"
    ...
    def _login(self):
        ...
        if role:
            ...
            self.err.set("❌  Wrong username or password. Try again.")
            self._p.set("")
...
class UserManagerDialog(tk.Toplevel):
    def __init__(self, parent, T):
        ...
        self.geometry(str(w) + "x" + str(h) + "+" +
                      str(px - w // 2) + "+" + str(py - h // 2))

    def _build(self):
        T = self.T
        tk.Label(self, text="👥  Manage Users",
        ...
        tk.Label(lf, text="Current Users:", font=("Segoe UI", 9, "bold"),
        ...
        tk.Button(lf, text="🗑  Remove Selected User",
                  font=("Segoe UI", 8), bg=T["RED"], fg="white",
                  relief="flat", cursor="hand2",
        ...
        for lbl, var, show in [("Username", self._nu, ""), ("Password", self._np, "●")]:
            ...
            tk.Entry(row, textvariable=var, show=show, font=("Segoe UI", 9),
                     ...
                     ).pack(side="right", expand=True, fill="x", ipady=4)
        tk.Button(af, text="➕  Add User",
                  font=("Segoe UI", 10, "bold"), bg=T["ACCENT2"],
    ...
    def _refresh(self):
        ...
        for u, d in load_users().items():
            if u == DEVELOPER_USER:
                continue   # never show developer account
    ...
    def _add(self):
        ...
        if not u or not p:
            ...
            return
        if u == DEVELOPER_USER:
            self.msg.set("⚠  That username is reserved.")
            return
        users = load_users()
        if u in users:
            self.msg.set("⚠  User '" + u + "' already exists.")
            return
        users[u] = {"hash": hash_pw(p), "role": "accountant"}
        save_users(users)
...
class ChangePasswordDialog(tk.Toplevel):
    ...
    def _build(self):
        ...
        tk.Button(card, text="✅  Save New Password",
                  font=("Segoe UI", 10, "bold"), bg=T["ACCENT2"],
                  fg="white", relief="flat", cursor="hand2", command=self._change
    ...
    def _change(self):
        ...
        # Allow developer to change their own password too
        if self.current_user == DEVELOPER_USER:
            if hash_pw(old) != hash_pw(DEVELOPER_PASS):
                self.msg.set("⚠  Current password is incorrect.")
                return
        elif not u or u["hash"] != hash_pw(old):
            self.msg.set("⚠  Current password is incorrect.")
            return
        if len(n1) < 6:
...
class EditEmployeeDialog(tk.Toplevel):
    ...
    def _build(self):
        ...
        self._sal_var  = tk.StringVar(value=str(r["salary"]))
        self._all_var  = tk.StringVar(value=str(r["allowance"]))
        ...
        for label, var, show in [
            ...
            ("💰  Basic Salary (TZS)", self._sal_var,  ""),
            ("➕  Allowance (TZS)",   self._all_var,  ""),
        ]:
            tk.Label(card, text=label, font=("Segoe UI", 9, "bold"),
                     bg=T["PANEL"], fg=T["TEXT"], anchor="w").pack(anchor="w", pady=(8, 1))
            e = tk.Entry(card, textvariable=var, show=show, font=("Segoe UI", 11),
                         bg=T["ENTRY_BG"], fg=T["ENTRY_FG"],
                         insertbackground=T["TEXT"], relief="flat", bd=4, width=30)
            e.pack(fill="x", ipady=5)

        tk.Frame(card, bg=T["SEP"], height=1).pack(fill="x", pady=(12, 6))

        lq = tk.Frame(card, bg=T["PANEL"])
        lq.pack(fill="x")
        tk.Label(lq, text="Has loan deduction?",
                 font=("Segoe UI", 9), bg=T["PANEL"], fg=T["TEXT"]).pack(side="left")
        ...
        tk.Frame(card, bg=T["ACCENT"], height=1).pack(fill="x", pady=(12, 4))
        self._prev_var = tk.StringVar(value="")
        tk.Label(card, textvariable=self._prev_var, font=("Segoe UI", 9),
                 bg=T["PANEL"], fg=T["ACCENT"], justify="left").pack(anchor="w")
        ...
        for var in (self._sal_var, self._all_var, self._loan_var):
    ...
    def _n(self, var):
        try:
            return float(var.get().replace(",", "").strip() or 0)
        except Exception:
            return 0.0

    def _preview(self, *_):
        s = self._n(self._sal_var)
        if s <= 0:
            self._prev_var.set("Enter a salary to see calculated values.")
            return
        a  = self._n(self._all_var)
        la = self._n(self._loan_var) if self._has_loan.get() else 0.0
    ...
    def _save(self):
        ...
        if self._has_loan.get() and la <= 0:
            ...
            return
        allowance = self._n(self._all_var)
        nssf, pay, paye, gross, ld, lb, net, amt = calculate(
            salary, allowance, self._has_loan.get(), la, self._has_lb.get())
        updated = dict(name=name, salary=salary, nssf=nssf, pay=pay, paye=paye,
...
class PayrollApp(tk.Tk):
    ...
    def _build_ui(self):
        ...
        self._build_totals_bar(right_wrap)
    ...
    def _build_header(self):
        ...
        # Left — Actions button + Month entry
        ...
        tk.Label(mid,
                 text="Tanzania  ·  TRA PAYE 2024/25  ·  v" + VERSION +
        ...
        if self.role == "admin":
            ...
            ToolTip(b, "Add or remove user accounts")
        ...
        ToolTip(b3, "Check GitHub for a newer version of this app")
    ...
    def _build_actions_menu(self):
        ...
        m = tk.Menu(self, tearoff=0,
                    ...
                    font=("Segoe UI", 10),
                    relief="flat", bd=1)
        m.add_command(label="  💾   Save  (same file)",         command=self.save_excel)
        m.add_command(label="  💾   Save As  (choose location)", command=self.save_excel_as)
        m.add_command(label="  🖨    Print Payroll",             command=self.print_payroll)
        m.add_command(label="  📂   Open Last Excel",            command=self.open_file)
        m.add_separator()
        m.add_command(label="  🔄   New Session  (clear list)",  command=self.new_session)
    ...
    def _build_form(self, p):
        ...
        for af, ck, tip in [
            ...
            ("P.A.Y.E",           "RED",     "Income tax (TRA 2024/25)"),
            ("GROSS PAY",         "GOLD",    "Salary + Allowance"),
            ("LOAN BOARD",        "SUBTEXT", "15% of salary if ticked"),
            ("NET PAY",           "ACCENT",  "After NSSF and PAYE"),
            ("AMOUNT TO BE PAID", "ACCENT",  "What employee receives"),
        ]:
    ...
    def _big_field(self, parent, label, key, hint=""):
        ...
        tk.Label(f, text=label, anchor="w",
                 font=("Segoe UI", 9, "bold"), bg=T["PANEL"], fg=T["TEXT"]).pack(anchor="w")
        var = tk.StringVar()
    ...
    def _build_table(self, p):
        ...
        tk.Label(hrow, textvariable=self.count_var,
                 font=("Segoe UI", 8), bg=T["PANEL"], fg=T["SUBTEXT"]
                 ).pack(side="right", padx=4, pady=(8, 2))
        ...
        for c in HEADERS:
            ...
            self.tree.column(c, width=80, anchor="center", minwidth=50, stretch=True)
    ...
    def _fit_columns(self, event=None):
        if not hasattr(self, "tree"):
            return
        try:
            total_w = self.tree.winfo_width() - 20
            if total_w <= 0:
                return
    ...
    def _refresh_totals_bar(self):
        if not hasattr(self, "_tot_vars"):
            return
        if not self.records:
            for v in self._tot_vars.values():
                v.set("—")
            return
    ...
    def save_excel(self):
        if not self.records:
            ...
            return
        # Same session — save to the same file without asking again
    ...
    def save_excel_as(self):
        ...
        try:
            save_to_excel(self.records, fp, self.username, self.month_var.get())
            self._last_saved_path = fp
            messagebox.showinfo("✅  Saved", "Payroll saved to:\n\n" + fp)
        except Exception as e:
            messagebox.showerror("❌  Error saving", str(e))
    ...
    def _auto_update_check(self):
        """Silent background check on every login. Prompts only if update found."""
        def on_done(result):
    ...
    def _manual_update_check(self):
        """User clicked ⬆ Update — always shows a result."""
        if not GITHUB_READY:
            messagebox.showinfo("Update Not Configured",
                "GitHub is not configured yet.\n\n"
                "Open payroll_app.py and set GITHUB_USER and GITHUB_REPO "
                "at the top of the file to enable automatic updates.")
            return

        def on_done(result):
            if result == "same":
                msg = ("You already have the latest version (v" + VERSION + ")."
                       if IS_FROZEN
                       else "Your code matches the version on GitHub. No update needed.")
    ...
    def _prompt_update(self, update_data):
        """Show Update Now / Do It Later dialog."""
        ver_label = (
            "Version " + str(update_data) + " is available."
            if IS_FROZEN
            else "A code update has been found on GitHub."
        )
        ...
        sw, sh = self.winfo_screenwidth(), self.winfo_screenheight()
        dlg.geometry("440x260+" + str((sw - 440) // 2) + "+" + str((sh - 260) // 2))

        tk.Label(dlg, text="🔄  Update Available",
                 font=("Segoe UI", 14, "bold"), bg="#1e2a3a", fg="#2ecc71").pack(pady=(22, 4))
        tk.Label(dlg, text=ver_label,
                 font=("Segoe UI", 10), bg="#1e2a3a", fg="#ecf0f1").pack()
        tk.Label(dlg, text="Would you like to update now, or do it later?",
                 font=("Segoe UI", 9), bg="#1e2a3a", fg="#95a5a6").pack(pady=(4, 20))

        def do_update():
            dlg.destroy()
            apply_update(update_data, self)

        def do_later():
            dlg.destroy()
            messagebox.showinfo("Reminder",
                "No problem!\n\n"
                "You can update any time using the  ⬆ Update  button in the top bar.")

        bf = tk.Frame(dlg, bg="#1e2a3a")
        bf.pack()
        tk.Button(bf, text="✅  Update Now",
                  font=("Segoe UI", 11, "bold"), bg="#27ae60", fg="white",
                  relief="flat", cursor="hand2", padx=16, pady=10,
                  command=do_update).pack(side="left", padx=10)
        tk.Button(bf, text="🕐  Do It Later",
                  font=("Segoe UI", 11), bg="#2d4059", fg="#ecf0f1",
                  relief="flat", cursor="hand2", padx=16, pady=10,
                  command=do_later).pack(side="left", padx=10)
        tk.Label(dlg, text="The app will restart automatically after updating.",
                 font=("Segoe UI", 7, "italic"), bg="#1e2a3a", fg="#64748b").pack(pady=(14, 0))
...
def start_app():
    if not has_any_users():
        # No accounts exist — ask what they want to do
        gate = FirstLaunchGate()
        gate.mainloop()

        if gate.choice == "create":
            setup = FirstSetupWindow()
            setup.mainloop()
            if not setup.created_user:
                return
            # Log in automatically right after account creation
            app = PayrollApp(setup.created_user, setup.created_role)
            app.mainloop()

        elif gate.choice == "login":
            # Developer / someone who already has credentials
            login = LoginWindow()
            login.mainloop()
    ...
    # Normal launch — accounts exist, go straight to login
    login = LoginWindow()
    login.mainloop()
    if login.logged_in_user:
        app = PayrollApp(login.logged_in_user, login.logged_in_role)
